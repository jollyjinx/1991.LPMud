...
list_members(ply)
              {
                ...
                update_members();
		if(LEVEL>=LEVEL_WIZARD) write(mcounter+" ");
...
recap_channel(str)
		...
		if(!test_bit(memberm[mom],a) && LEVEL<LEVEL_WIZARD )
		{ 
		  write("You are no listener of that channel.\n");
		  return 1; 
		}
...
update_sells()
...
#ifdef MONITOR
	   	write("Update_sell with hist_length :"+hist_length[2]+"\n");
#endif		
	  ...
	  write("dritte false\n");
			  ...
			  flag=a+1;
		        ...
		        {
		          if( splayer[a]->query_spell_points()<JJ_SELLCOSTS )
			  {
#ifdef MONITOR	
	    write("vierte false\n");
#endif
...
kill_sell(flag) 
	    ...
	    int a;	    
#ifdef MONITOR
	   	write("kill_sell with hist_length :"+hist_length[2]+"\n");
#endif		    
...
#ifdef MONITOR
	write("Historylength :"+hist_length[2]+"\n");
#endif
