...
quit() {
    save_me(0);
 if(level>19 && file_size("/players/"+name+"/workroom.c")>-1) /* by jolly */
    	move_object(this_object(),"players/"+name+"/workroom.c");
    drop_all(1);
...
valid_write(str) {
    ...
    if(sscanf(str, "/room/%s", who) == 1) {
	if (level < 24 )
	    return 0;
        return "room/" + who;
    }
    if(sscanf(str, "/obj/%s", who) == 1) {
	if (level < 24 )
	    return 0;
        return "obj/" + who;
    }
    if(sscanf(str, "/banish/%s", who) == 1) {
	if (level < 24 )
	    return 0;
        return "banish/" + who;
    }
    if(sscanf(str, "/doc/%s", who) == 1) {
	if (level < 24 )
	    return 0;
        return "doc/" + who;
    }
...
compute_auto_str() {
	...
	if( str[i]=='^' ) flag=1;
	if(!flag) auto_load = auto_load + str + "^!";
    }
}
